...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch providers
      const providersResponse = await fetch('http://localhost:5000/api/services/providers', {
        credentials: 'include'
      })
  ...
  const toggleProviderVerification = async (providerId, currentStatus) => {
    try {
      const response = await fetch(`http://localhost:5000/api/admin/providers/${providerId}/toggle-verification`, {
        method: 'POST',
        credentials: 'include'
      })
      if (response.ok) {
